...
from EAbotoy.model import EventMsg, FriendMsg, GroupMsg, WeChatMsg
...
class Action:
    ...
    @classmethod
    def from_ctx(
            cls, ctx: Union[EventMsg, FriendMsg, GroupMsg, WeChatMsg], timeout: int = 20
    ) -> "Action":
        return cls(
            ctx.CurrentWxid,
    ...
    def getClusterInfo(self) -> dict:
        """获取当前集群信息"""
        return self.get("", path="/v1/ClusterInfo", params={"isShow": 1, "qq": 1})
    ...
    def post(
        ...
        functools.update_wrapper(job, self.baseRequest)
        if self._use_queue:
