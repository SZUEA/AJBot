...
class Session(SessionBase):
    def __init__(self, ctx: WeChatMsg, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.ctx: WeChatMsg = ctx
        self.action = Action(
    ...
    def send_text(self, text: str):
        ...
        :param text: 文字内容
        """
        return self.action.sendWxText(self.ctx.FromUserName, text)

    def send_pic(
            self, url: str = "", base64: str = ""
    ):
        """发送session图片消息
        :param url: 图片URL
...
class SessionController:
    ...
    def define_session_id(
            self, ctx: WeChatMsg, single_user=True
    ) -> str:
        """定义session id
        :param ctx: ctx
        ...
        if single_user:
            sid = "{group_id}-{user_id}".format(
                group_id=ctx.FromUserName, user_id=ctx.ActionUserName
            )
        else:
            sid = str(ctx.FromUserName)
        return sid

    def remove_session(self, ctx: WeChatMsg, single_user: bool = True):
        """移除对应session"""
    ...
    def session_existed(
            self, ctx: WeChatMsg, single_user: bool = True
    ):
        """判断对应session是否存在"""
        sid = self.define_session_id(ctx, single_user)
