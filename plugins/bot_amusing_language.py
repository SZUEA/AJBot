...
matrices = jconfig.amusing_language_matrices or ["嘉然", "贝拉", "打架", "唱歌", "向晚", 'rap', '乃琳', '摸腿', '珈乐', '是谁', ',']
...
def receive_group_msg(ctx: GroupMsg):
    if ctx.Content.startswith("翻译发病 "):
        msg = decode(ctx.Content[5:])
        if msg:
            Text(msg)
    elif ctx.Content.startswith("发病 "):
        Text(encode(ctx.Content[3:]))
